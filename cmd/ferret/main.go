...
func main() {
	...
	case "fb login":
		runFBLogin(ctx, os.Args[3:])
	case "fb check":
		runFBCheck(os.Args[3:])
	case "fb search":
		runFBSearch(ctx, os.Args[3:])
	case "fb item":
		runFBItem(ctx, os.Args[3:])
	default:
		usage()
		os.Exit(1)
	}
...
Usage:
  ...
  ferret fb item        -ids A,B,C [-proxy URL]                          listing details (miles, title…)
